...
| `clify skills` | Generate AI agent skills from spec |
...
Clify generates agent skills alongside your CLI — markdown files that teach AI agents how to use it:

```bash
clify skills api.clify.yaml --output ./my-cli
```

This creates a `skills/` directory with shared auth patterns, per-group command references, and per-command action guides. Compatible with any agent framework that reads SKILL.md files (e.g., OpenClaw).

**Clify's own agent skill** is at [`skills/clify/SKILL.md`](skills/clify/SKILL.md) — install it so your AI assistant can help you build CLIs conversationally.
